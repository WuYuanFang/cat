...
- (instancetype)initWithConfig:(CMPageTitleConfig *)config {
    ...
    if (self = [super init]) {
        ...
        if (config.layerArr.count > 0) {
            // wxq_渐变色
            for (CALayer *layer in config.layerArr) {
...
- (void)clickLabel:(UIGestureRecognizer *)tap {
    ...
    // wxq_ 改动
    CMDisplayTitleLabel *label = nil;
    if (tap) {
        label = (CMDisplayTitleLabel *)tap.view;
    }else {
        if (self.titleLabels.count > 0) {
            label = self.titleLabels[self.config.cm_selectedIndex];
        }
    }
    // 获取对应标题label
