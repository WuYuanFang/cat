...
- (void)xq_setText:(NSString *)text {
    ...
    if ([self.searchBar canBecomeFocused]) {
        [self.searchBar becomeFirstResponder];
        
        //刷新模糊搜索界面
        if (self.delegate && [self.delegate respondsToSelector:@selector(searchViewController:refreshWithKeyString:dataBlock:)]) {
            @weakify(self)
            [self.delegate searchViewController:self refreshWithKeyString:self.searchBar.text dataBlock:^(NSArray <XQZQSearchModel *> *dataArr) {
                @strongify(self)
                [self.ctr2 refreshSearchEditViewWith:dataArr];
            }];
        }
...
- (void)pop {
    ...
    [self.delegate searchViewControllerDidSelectCancel:self];
}
...
- (void)textFieldDidExit:(UITextField *)field {
    //历史记录
    XQZQSearchModel *sModel = [XQZQSearchModel new];
    sModel.title = field.text;
    [self saveSearchCacheWith:sModel];
...
//        [self.delegate searchFuzzyResultWithKeyString:field.text data:nil resultController:nil];
//    }
    
    @weakify(self)
    [self.delegate searchViewController:self tapSearchWithKeyString:self.searchBar.text dataBlock:^(NSArray <XQZQSearchModel *> *dataArr) {
        @strongify(self)
        [self.ctr2 refreshSearchEditViewWith:dataArr];
    }];
...
#pragma mark - private
- (void)saveSearchCacheWith:(XQZQSearchModel *)searchModel {
    if (searchModel.title.length > 0) {
        NSArray *arr = [XQPredicate predicateKeyEqWithDataArr:self.ctr3.historyList value:searchModel.title key:@"title"];
        
        if (arr.count > 0) {
            [self.ctr3.historyList removeObjectsInArray:arr];
            [self.ctr3.historyList insertObject:arr.firstObject atIndex:0];
        }else {
//            XQZQSearchModel *model = [XQZQSearchModel new];
//            model.title = searchText;
            [self.ctr3.historyList insertObject:searchModel atIndex:0];
        }
        [XQZQSearchModel xq_setHistoryListWithModelArr:self.ctr3.historyList];
...
- (void)searchChildViewDidSelectItem:(XQZQSearchModel *)value {
    // wxq 点击热门搜索 和 历史记录
    //保存历史
...
//    [self searchEditViewNeedRefresh:@(self.inputCount)];
    ...
    //刷新结果界面
    if (self.delegate && [self.delegate respondsToSelector:@selector(searchViewController:resultData:)]) {
        [self.delegate searchViewController:self resultData:value];
    }
}

- (void)searchChildViewDidSelectRow:(XQZQSearchModel *)value {
    // wxq 激活输入框, 点击搜索出来的内容
...
- (UIView *)leftView {
    ...
    UIImageView *imgView = [[UIImageView alloc] initWithImage:[XQZQImage imageNamed:@"ic_search@3x"]];
...
- (UIButton *)backBtn {
    if (!_backBtn) {
        ...
        [_backBtn setImage:[XQZQImage imageNamed:@"ic_back@2x"] forState:UIControlStateNormal];
