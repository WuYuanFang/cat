...
/// 显示减按钮
- (void)xq_showDecreaseBtn {
    [UIView animateWithDuration:0.3f animations:^{
        self.decreaseBtn.alpha = 1;
        self.decreaseBtn.frame = CGRectMake(0, 0, _height, _height);
...
/// 隐藏减按钮
- (void)xq_hideDecreaseBtn {
    [UIView animateWithDuration:0.3f animations:^{
        self.decreaseBtn.alpha = 0;
        self.decreaseBtn.frame = CGRectMake(_width-_height, 0, _height, _height);
    }];
}
...
- (void)decrease
{
    ...
    if (number >= _minValue) {
        ...
        if (_decreaseHide && number<_minValue) {
            ...
            [self xq_hideDecreaseBtn];
            
            return;
        }
        
        /// wxq, 最小, 也要返回出去
        if ([_delegate respondsToSelector:@selector(pp_numberButton:minValueNumber:)]) {
            [_delegate pp_numberButton:self minValueNumber:_textField.text.floatValue];
        }
