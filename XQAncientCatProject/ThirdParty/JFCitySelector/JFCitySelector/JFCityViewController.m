...
#define JFCity_Def_inLocation @"正在定位..."
...
@property (nonatomic, assign) CGFloat cellHeight;                   //添加的(显示区县名称)cell的高度;
...
- (void)startLoation {
    ...
    [self.locationManager startPositioningSystem];
}
...
//    [leftButton setTitleColor:[UIColor blackColor] forState:UIControlStateNormal];
    self.navigationItem.leftBarButtonItem = [[UIBarButtonItem alloc] initWithTitle:@"返回" style:UIBarButtonItemStylePlain target:self action:@selector(backrootTableViewController)];
}
    ...
    if ([[cityDic valueForKey:@"cityName"] isEqualToString:@"全城"]) {
        ...
        [_manager currentCity:[kCurrentCityInfoDefaults objectForKey:@"cityNumber"] currentCityName:^(NSString *name) {
            ...
            if (strongSelf) {
                ...
                [self xq_callbackWithCityName:name];
            }
        }];
        ...
        [self xq_callbackWithCityName:cityName];
...
- (JFSearchView *)searchView {
    if (!_searchView) {
        ...
        _searchView = [JFSearchView new];
        _searchView.view.frame = CGRectMake(0, 104, frame.size.width, frame.size.height  - 104);
        _searchView.view.backgroundColor = [UIColor colorWithRed:155 / 255.0 green:155 / 255.0 blue:155 / 255.0 alpha:0.5];
...
- (void)deleteSearchView {
    [_searchView.view removeFromSuperview];
    _searchView = nil;
}
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
    if (indexPath.section < _HeaderSectionTotal) {
        ...
        if (indexPath.section == _HeaderSectionTotal - 3) {
            ...
            _cell.cityNameArray = locationCity ? @[locationCity] : @[JFCity_Def_inLocation];
        }
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    [self xq_callbackWithCityName:cell.textLabel.text];
}
...
- (void)searchResult:(NSString *)result {
    [_manager searchCityData:result result:^(NSMutableArray *result) {
        if ([result count] > 0) {
            self.searchView.view.backgroundColor = [UIColor whiteColor];
            self.searchView.resultMutableArray = result;
        }
    }];
}
...
- (void)searchResults:(XQ_JFAreaDataModel *)dic {
    [kCurrentCityInfoDefaults setObject:dic.city_name forKey:@"currentCity"];
    [kCurrentCityInfoDefaults setObject:dic.city_number forKey:@"cityNumber"];
    NSString *nameStr = dic.city_name;
    
    [self xq_callbackWithCityName:nameStr];
    
    [self dismissViewControllerAnimated:YES completion:nil];
    [self historyCity:dic.city_name];
}
...
- (void)xq_callbackWithCityName:(NSString *)cityName {
    if ([cityName isEqualToString:JFCity_Def_inLocation]) {
        return;
    }
    
    [self historyCity:cityName];
    
    if ([self.delegate respondsToSelector:@selector(cityViewControllercity:cityName:)]) {
        [self.delegate cityViewControllercity:self cityName:cityName];
    }
    
    if ([self.delegate respondsToSelector:@selector(cityViewControllercity:cityInfo:)]) {
        NSArray <XQ_JFAreaDataModel *> *arr = [[JFAreaDataManager shareInstance] xq_queryCityInfoWithCityName:cityName];
        if (arr.count > 0) {
            [self.delegate cityViewControllercity:self cityInfo:arr.firstObject];
        }
    }
