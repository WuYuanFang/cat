...
- (NSArray <NSNumber *> *)allSelectedIndexTags {
    NSMutableArray *allTags = [NSMutableArray new];
    
    for (TTGTextTagLabel *label in _tagLabels) {
        if (label.selected) {
            NSNumber *labelIndex = [[NSNumber alloc] initWithInt:label.labelIndex];
            [allTags addObject:[labelIndex copy]];
        }
    }
...
- (void)tagCollectionView:(TTGTagCollectionView *)tagCollectionView didSelectTag:(UIView *)tagView atIndex:(NSUInteger)index {
    if (_enableTagSelection) {
        ...
        //自定义单选的展现方式
        int selectCount = 0;
        if(_selectionLimit == 1){
            for (TTGTextTagLabel *tmplabel in _tagLabels) {
                if (tmplabel.selected && label != tmplabel) {
                    selectCount += 1;
                    tmplabel.selected = NO;
                }
            }
